...
public class PeFACConnector {
	private static final Log LOG = LogFactory.getLog(PeFACConnector.class);
	
	public PeFACConnector(String urlFACeEndpoint, String certificateAlias, char[] certificatePassword, boolean useProxy, String proxyHost, String proxyPort,  boolean authProxy, String proxyUser,String proxyPassword,String storeFile, char[] storePassword, FACeCertificateProvider certificateProvider) {
	...
	public SSPPResultadoEnviarFactura enviarFactura(SSPPFactura request) throws FACeException {
		try {
			if(request != null && request.getFichero_factura() != null && (request.getFichero_factura().getMime()==null || request.getFichero_factura().getMime().length()==0)) {
	...
	public  SSPPResultadoConsultarUnidades consultarUnidades() throws FACeException {
		try {
			SSPPWebServiceProxyPort webservice = new SSPPWebServiceProxyServiceLocator().getSSPPWebServiceProxyPort(new URL(FACeConfig.getInstance().getUrlFACeEndpoint()));
