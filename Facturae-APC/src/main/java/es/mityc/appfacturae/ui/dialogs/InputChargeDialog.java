...
public class InputChargeDialog extends JDialog {
	
	private static Log logger = LogFactory.getLog(InputChargeDialog.class);
	...
	public InputChargeDialog (Frame parent, boolean modal, Double total, String version, boolean global) {
		super(parent, modal);
	    this.total = total;
	...
	private void init() {
		...
		// Si el cargo es global, el porcentaje es obligatorio (no se permiten cargos por cantidad directa)
		if (global)
		...
		PercentValidator pv = new PercentValidator();
		percentTextField.addFocusListener(pv);
		...
		// Si el cargo es global, no se permiten cargos por cantidad directa (se calculan en función del porcentaje)
		if (global){
		...
		if (global)
			setTitle(Constants.LANG.getString("GlobalChargeAddTitle"));
		else
			setTitle(Constants.LANG.getString("ChargeAddTitle"));
	...
	private class ConceptValidator extends FocusAdapter {
		public void focusLost(FocusEvent evt){
			...
			// Concept value may be a 2500 character max. string
	...
	private class PercentValidator extends FocusAdapter {
		public void focusLost(FocusEvent evt){
			...
			if (value != null && !"".equals(value)) {
				...
				try{
					...
					doubleObj = DoubleUtil.round(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge_rate")));
				...
				// It establishes the total amount, calculated from total
				doubleObj = (total * doubleObj.doubleValue()) / 100;
				doubleObj = DoubleUtil.round(doubleObj, Integer.parseInt(decimalProps.getProperty("item_charge")));
	...
	private class AmountValidator extends FocusAdapter {
		public void focusLost(FocusEvent evt){
			...
			if (value != null && !"".equals(value)) {
				...
				amountExpected = DoubleUtil.round(amountExpected, Integer.parseInt(decimalProps.getProperty("item_charge")));
