...
import es.mityc.appfacturae.facturae321.InvoiceLineType.TaxesOutputs.Tax;
...
import es.mityc.appfacturae.ui.dialogs.Invoice321DetailDialog;
...
import es.mityc.appfacturae.ui.utils.JNumericField;
import es.mityc.appfacturae.ui.windows.GenerateInvoice321Window;
...
public class InvoiceDetail321Panel extends JPanel {

	private static final long serialVersionUID = -2704987015456769574L;
	private Properties decimalProps = null;
	private static final String codeVersion = Constants.FACTURAE321;
	private static Log logger = LogFactory.getLog(InvoiceDetail321Panel.class);
	private static String version = Constants.VERSION321;

	private boolean setVisualizationOnly = false;

	public InvoiceDetail321Panel() {
		super();
	...
	private void initComponents() {
		...
		jTextFieldWithPerc = new JNumericField();
		((JNumericField) jTextFieldWithPerc)
			.setPrecision(Integer.parseInt(decimalProps.getProperty("item_charge_rate")));
		...
		if(literalsListDestModel==null) 
			literalsListDestModel = new DefaultListModel();
		jListLiteralsDest = new JList(literalsListDestModel);
...
// Inerza S.A. - Añadir endosatario v321 - #0062026
		...
		jTableGlobDisc.setModel(new NoEdiTableModel(new Object[][] {

		}, new String[] { Constants.LANG.getString("Item"), Constants.LANG.getString("Rate"),
		...
		jTableGlobDisc.getColumnModel().getColumn(0).setCellRenderer(
		...
		jTableGlobDisc.getColumnModel().getColumn(1).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
		...
		jTableGlobDisc.getColumnModel().getColumn(2).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		...
		jButtonDiscAdd.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseEntered(java.awt.event.MouseEvent evt) {
				InvoiceDetail321Panel.this.mouseEntered(evt);
			}

			public void mouseExited(java.awt.event.MouseEvent evt) {
				InvoiceDetail321Panel.this.mouseExited(evt);
			}
		});
			...
			public void focusLost(FocusEvent evt) {
				...
				if (value != null && !"".equals(value.trim())) {
					if (value3 == null || "".equals(value3.trim())) {
						...
						((GenerateInvoice321Window) jTextFieldWithPerc.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("ParameterRequired"), Constants.ERROR_MSG_COLOR);
					} else {
						// Amount may be a 2500 max. String
						if (value == null || value.length() > 2500) {
							((JTextField) evt.getSource()).setText("");
							((GenerateInvoice321Window) jTextFieldWithItem.getTopLevelAncestor()).showMessage(
		...
		jTextFieldWithPerc.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value2 != null && !"".equals(value2)) {
					...
					try {
						doubleObj = DoubleUtil.getDoubleFromPrettyNumber(value2);
						...
						//BigDecimal bdObj = new BigDecimal(doubleObj);
						//bdObj = DoubleUtil.roundWithPrecision(doubleObj,
							//Integer.parseInt(decimalProps.getProperty("item_charge_rate")));
						((JTextField) evt.getSource()).setText(DoubleUtil.roundByVersionFormatted(doubleObj,
								Integer.parseInt(decimalProps.getProperty("item_charge_rate")),version));
					} catch (Exception e) {
						((JTextField) evt.getSource()).setText("");
						((GenerateInvoice321Window) jTextFieldWithPerc.getTopLevelAncestor()).showMessage(
					...
					if (totalMsg != null && !"".equals(totalMsg)) {
						Double total = DoubleUtil.getDoubleFromPrettyNumber(totalMsg);
						doubleObj = (total * doubleObj.doubleValue()) / 100;
						// Negative invoices are allowed
						// if (doubleObj.doubleValue() < 0){
						// 		doubleObj = 0.0;
						// 		((GenerateInvoice321Window)jTextFieldWithPerc.getTopLevelAncestor()).
						//			showMessage(Constants.LANG.getString("NOOKParamNegative"), Constants.ERROR_MSG_COLOR);
						// }
						jTextFieldWithAmou.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(doubleObj,
							Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
						jTextFieldWithAmou.setBackground(Color.white);
						jTextFieldResults[7].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(doubleObj,
							Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
					} else {
		...
		jTextFieldWithAmou.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				String value = jTextFieldWithItem.getText();
				String value3 = jTextFieldWithAmou.getText();
				if (value != null && !"".equals(value.trim()) && (value3 == null || "".equals(value3.trim()))) {
					jTextFieldWithAmou.setBackground(Constants.BKG_ERROR_COLOR);
					((GenerateInvoice321Window) jTextFieldWithAmou.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("ParameterRequired"), Constants.ERROR_MSG_COLOR);
				} else if (value3 != null && !"".equals(value3.trim())) {
					if (value == null || "".equals(value.trim())) {
						jTextFieldWithItem.setBackground(Constants.BKG_ERROR_COLOR);
					...
					Double doubleObj = null;
					try {
						doubleObj = DoubleUtil.getDoubleFromPrettyNumber(value3);
						((JTextField) evt.getSource()).setText(DoubleUtil.roundWithPrecisionDoubleFormatted(doubleObj,
							Integer.parseInt(decimalProps.getProperty("global_withheld"))));
						jTextFieldResults[7].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(doubleObj,
							Integer.parseInt(decimalProps.getProperty("global_withheld"))));
					...
					if (value2 != null && !"".equals(value2.trim())) {
						Double rateReaded = DoubleUtil.getDoubleFromPrettyNumber(value2); // %
						String totalMsg = jTextFieldResults[6].getText(); // Total cost
						Double total = null;
						if (totalMsg != null && !"".equals(totalMsg))
							total = DoubleUtil.getDoubleFromPrettyNumber(totalMsg);
						Double amountExpected = (total * rateReaded.doubleValue()) / 100; // Amount expected
						amountExpected = DoubleUtil.roundWithPrecisionDouble(amountExpected,
							Integer.parseInt(decimalProps.getProperty("global_withheld")));
						Double amountReaded = null;
						amountReaded = DoubleUtil.getDoubleFromPrettyNumber(value3); // Amount readed
		...
		jPanel21Layout.setHorizontalGroup(jPanel21Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel21Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jPanel21Layout.createSequentialGroup()
							.add(jLabelTotalDisc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
							.add(jTextFieldTotalDisc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 100,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(jScrollPaneGlobDisc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 887,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
			.add(5, 5, 5)
			.add(jPanel21Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jButtonDiscDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
					.add(jButtonDiscAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)).add(18, 18, 18));
		jPanel21Layout.setVerticalGroup(jPanel21Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel21Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
					.add(jScrollPaneGlobDisc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 50,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jPanel21Layout.createSequentialGroup()
							.add(jButtonDiscAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
			...
			.add(jPanel21Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
					.add(jTextFieldTotalDisc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jLabelTotalDisc)).addContainerGap());
		...
		jTableGlobReim.setModel(new NoEdiTableModel(new Object[][] {

		}, new String[] { Constants.LANG.getString("IssuerType"), Constants.LANG.getString("ResidenceType"),
				Constants.LANG.getString("TaxIdentificationNumber"), Constants.LANG.getString("ReceiverType"),
				Constants.LANG.getString("ResidenceType"), Constants.LANG.getString("TaxIdentificationNumber"),
				Constants.LANG.getString("Amount") + " (" + Constants.LANG.getString("EuroSimbol") + ")" 
				},
			new int[] {}));
		...
		jTableGlobReim.getColumnModel().getColumn(0).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(1).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(1).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(2).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(2).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(3).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(3).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(4).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(4).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(5).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(5).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobReim.getColumnModel().getColumn(6).setResizable(false);
		jTableGlobReim.getColumnModel().getColumn(6).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		...
		jPanel23Layout.setHorizontalGroup(jPanel23Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel23Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jPanel23Layout.createSequentialGroup()
							.add(jLabelTotalReim, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
							.add(jTextFieldTotalReim, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 100,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(jScrollPaneGlobReim, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 887,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
			.add(5, 5, 5)
			.add(jPanel23Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jButtonReimDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
					.add(jButtonReimAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)).add(18, 18, 18));
		jPanel23Layout.setVerticalGroup(jPanel23Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel23Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
					.add(jScrollPaneGlobReim, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 50,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jPanel23Layout.createSequentialGroup()
							.add(jButtonReimAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
			...
			.add(jPanel23Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
					.add(jTextFieldTotalReim, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jLabelTotalReim)).addContainerGap());
		...
		jTableGlobChar.getColumnModel().getColumn(0).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableGlobChar.getColumnModel().getColumn(1).setResizable(false);
		jTableGlobChar.getColumnModel().getColumn(1).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		jTableGlobChar.getColumnModel().getColumn(2).setResizable(false);
		jTableGlobChar.getColumnModel().getColumn(2).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		...
		jPanel15Layout.setHorizontalGroup(jPanel15Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel15Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jPanel15Layout.createSequentialGroup()
							.add(jLabelTotalChar, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
							.add(jTextFieldTotalChar, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 100,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(jScrollPaneGlobChar, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 887,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
			.add(5, 5, 5)
			.add(jPanel15Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jButtonCharDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
					.add(jButtonCharAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)).add(18, 18, 18));
		jPanel15Layout.setVerticalGroup(jPanel15Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel15Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
					.add(jScrollPaneGlobChar, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 50,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jPanel15Layout.createSequentialGroup()
							.add(jButtonCharAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
			...
			.add(jPanel15Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
					.add(jTextFieldTotalChar, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jLabelTotalChar)).addContainerGap());
		...
		jTableGlobAttc.setModel(new NoEdiTableModel(new Object[][] {

		}, new String[] { Constants.LANG.getString("Format"), Constants.LANG.getString("Path"),
				Constants.LANG.getString("Description"), Constants.LANG.getString("IncludeInXML") }, new int[] {}));
		...
		jButtonXMLAddData.addActionListener(new java.awt.event.ActionListener() {
			...
			private void verifyXML(ActionEvent evt) {
				if (jTextAreaXMLExtAddData.getText() == null || "".equals(jTextAreaXMLExtAddData.getText().trim())) {
					error = "";
					jButtonXMLAddDataZoom
						.setIcon(new ImageIcon(getClass().getResource("/images/button_mini_zoom.jpg")));
				} else {
					// Valid Invoice 3.1
					InputStream is = this.getClass().getResourceAsStream(Constants.I321_EMPTY_PATH);
					...
					try {
						...
						jButtonXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
							"/images/button_mini_zoom_error.jpg")));
						error = ve.getMessage();
						((GenerateInvoice321Window) jButtonXMLAddData.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKMessageExtensionAdded"), Constants.ERROR_MSG_COLOR);
					...
					if (f != null) {
						try {
							...
							vu.validate(f, Constants.VERSION321);
							error = "";
							jButtonXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
								"/images/button_mini_zoom.jpg")));
							((GenerateInvoice321Window) jButtonXMLAddData.getTopLevelAncestor()).showMessage(
								Constants.LANG.getString("OKMessageExtensionAdded"), Constants.OK_MSG_COLOR);
						} catch (ValidationException ve) {
							jButtonXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
								"/images/button_mini_zoom_error.jpg")));
							error = ve.getMessage();
		...
		jButtonXMLAddDataZoom.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				InputExtensionDialog ied = new InputExtensionDialog((JFrame) jTextAreaXMLExtAddData
					.getTopLevelAncestor(), true, jTextAreaXMLExtAddData.getText(), error);
				ied.setVisible(true);
		...
		jLabelXMLAddData = new JLabel();
		jLabelXMLAddData.setText(Constants.LANG.getString("AdditionalInformation") + " - "
			+ Constants.LANG.getString("Extension") + " (XML)");
		...
		jPanel24Layout.setHorizontalGroup(jPanel24Layout.createSequentialGroup()
			.addContainerGap()
			.add(jScrollPaneGlobAttc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 530,
				org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
			.add(jPanel24Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jButtonAttcDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
					.add(jButtonAttcAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE))
			.add(10, 10, 10)
			.add(jPanel24Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jLabelXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 300,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jScrollPaneXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 300,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
			.add(jPanel24Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
					.add(jButtonXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)
					.add(jButtonXMLAddDataZoom, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, Short.MAX_VALUE)).add(10, 10, 10));
		jPanel24Layout.setVerticalGroup(jPanel24Layout.createSequentialGroup()
			.addContainerGap()
			.add(jPanel24Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
					.add(jScrollPaneGlobAttc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 75,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(jPanel24Layout.createSequentialGroup()
							...
							.add(jButtonAttcDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								...
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(jPanel24Layout.createSequentialGroup()
							.add(jLabelXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 15,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jScrollPaneXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 60,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(jPanel24Layout.createSequentialGroup()
							.add(jButtonXMLAddData, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
			...
			public void focusLost(FocusEvent evt) {
				...
				if (value != null && !"".equals(value)) {
					...
					try {
						...
						((JTextField) evt.getSource()).setText(DoubleUtil.roundWithPrecisionDoubleFormatted(doubleObj,
								Integer.parseInt(decimalProps.getProperty("item_payment_amount"))));
					} catch (NumberFormatException nfe) {
						if (((JTextField) evt.getSource()).getText() != null
							&& !"".equals(((JTextField) evt.getSource()).getText().trim())) {
							((JTextField) evt.getSource()).setText("");
							((GenerateInvoice321Window) jTextFieldPaymentAmo.getTopLevelAncestor()).showMessage(
								Constants.LANG.getString("NOOKParamNumericFormat"), Constants.ERROR_MSG_COLOR);
						}
						return;
					}
		...
		String[] arrayPaymentMeansModel = Constants.APP_PROP.getProperty("paymentMeansCombo" + Constants.FACTURAE31)
			.split(";");
		...
		jTextFieldPaymentRefIssuer.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPaymentRefIssuer.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPaymentRefReceiver.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPaymentRefReceiver.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jComboBoxPayment.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (((JComboBox) evt.getSource()) != null && ((JComboBox) evt.getSource()).getSelectedItem() != null
					&& !"".equals(((JComboBox) evt.getSource()).getSelectedItem().toString())) {
					String valueId = ((JComboBox) evt.getSource()).getSelectedItem().toString().split(":")[1].trim();

					/** 
					 *  There is a bug in Hibernate using Aliases. It is not possible to query
					 *  the account to be debited and the account to be credited at the same
					 *  time. Firstly, the account to be debited will be asked. Then in another
					 *  query, the account to be credited
					 **/

					String query = "SELECT DUE_DATE,AMOUNT,MEAN,RECONCILIATION_REF,ADDITIONAL_INFO,ACCOUNT_TO_BE_DEBITED,ACCOUNT_TO_BE_CREDITED,DEBIT_RECONCILIATION_REF FROM PAYMENT_DETAIL WHERE PAYMENT_ID='"
						+ valueId + "'";
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						...
						if (values[6] != null) {
							...
							if (ls3 != null && ls3.size() > 0 && ls3.get(0) != null) {
								...
								if (values2[0] != null && values2[1] != null)
									jComboBoxPayAccountC.setSelectedItem(values2[0] + ": " + values2[1]);
							}
						}
						if (values[7] != null)
		...
		((JFormattedTextField) ((JSpinner) jCalendarComboBoxPaymDate.getComponent(0)).getEditor().getComponent(0))
			.addFocusListener(new FocusAdapter() {
				public void focusLost(FocusEvent evt) {
					if (!((JFormattedTextField) evt.getSource()).isEditValid())
						((GenerateInvoice321Window) jCalendarComboBoxPaymDate.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKMessageDateNotValid"), Constants.ERROR_MSG_COLOR);
				}
			});
		...
		org.jdesktop.layout.GroupLayout jPanelGlbPaymentGen2Layout = new org.jdesktop.layout.GroupLayout(
			jPanelGlbPaymentGen2);
		jPanelGlbPaymentGen2.setLayout(jPanelGlbPaymentGen2Layout);
		jPanelGlbPaymentGen2Layout
			.setHorizontalGroup(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
				.add(jPanelGlbPaymentGen2Layout.createSequentialGroup()
						.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
								.add(org.jdesktop.layout.GroupLayout.CENTER, jPanelGlbPaymentGen2Layout.createSequentialGroup()
										.addContainerGap()
										.add(5, 5, 5)
										.add(jLabelPayment, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 100,
											Short.MAX_VALUE)
										.add(5, 5, 5)
										.add(jComboBoxPayment, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 160,
											Short.MAX_VALUE).add(10, 10, 10))
								.add(org.jdesktop.layout.GroupLayout.CENTER,
									jPanelGlbPaymentGen2Layout.createSequentialGroup().addContainerGap().add(5, 5, 5)
										.add(jTextAreaPayExist, 0, 260, Short.MAX_VALUE)))
						.add(jSeparator1, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 2,
							org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
						.add(10, 10, 10)
						.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
								.add(jLabelTotalPayDate, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 100,
									Short.MAX_VALUE)
								.add(jLabelTotalPayMean, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 100,
									Short.MAX_VALUE))
						.add(5, 5, 5)
						.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
								.add(jCalendarComboBoxPaymDate, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 160,
									Short.MAX_VALUE)
								.add(jComboBoxPayMean, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 160,
									Short.MAX_VALUE))
						.add(20, 20, 20)
						.add(
							jPanelGlbPaymentGen2Layout
								.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
								.add(jLabelTotalPayRefIssuer, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 50,
									Short.MAX_VALUE)
								.add(jLabelTotalPayAmou, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 50,
									Short.MAX_VALUE))
						.add(5, 5, 5)
						.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
								.add(org.jdesktop.layout.GroupLayout.CENTER,jPanelGlbPaymentGen2Layout.createSequentialGroup()
										.add(jTextFieldPaymentRefIssuer,
											org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 60,
											org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
										.add(10, 10, 10)
										.add(jLabelTotalPayRefReceiver, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
											50, Short.MAX_VALUE)
										.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
										.add(jTextFieldPaymentRefReceiver,
											org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 60,
											org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
								.add(jTextFieldPaymentAmo, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 100,
									Short.MAX_VALUE)).add(20, 20, 20)));

		jPanelGlbPaymentGen2Layout.setVerticalGroup(jPanelGlbPaymentGen2Layout
			.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanelGlbPaymentGen2Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
							.add(jLabelPayment)
							.add(jComboBoxPayment, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(5, 5, 5)
					.add(jTextAreaPayExist, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
						org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(5, 5, 5))
			.add(org.jdesktop.layout.GroupLayout.TRAILING, jPanelGlbPaymentGen2Layout.createSequentialGroup()
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
					.add(jSeparator1, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 40,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addContainerGap())
			.add(jPanelGlbPaymentGen2Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
							.add(jLabelTotalPayDate)
							.add(jCalendarComboBoxPaymDate, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayRefIssuer)
							.add(jLabelTotalPayRefReceiver)
							.add(jTextFieldPaymentRefIssuer, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jTextFieldPaymentRefReceiver, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.add(5, 5, 5)
					.add(jPanelGlbPaymentGen2Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
							.add(jLabelTotalPayMean)
							.add(jComboBoxPayMean, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayAmou)
							.add(jTextFieldPaymentAmo, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)).add(5, 5, 5)));
		...
		jTextFieldPayDIBAN.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() < 5 && value.length() > 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDIBAN.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooLow"), Constants.ERROR_MSG_COLOR);
				}
				if (value.length() > 34) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDIBAN.getTopLevelAncestor()).showMessage(
		...
		jTextFieldPayDBank.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDBank.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayDBran.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDBran.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayDAddr.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 80) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDAddr.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayDPCod.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() != 5 && value.length() != 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDPCod.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamPostCodeLength"), Constants.ERROR_MSG_COLOR);
					return;
				}
				try {
		...
		jTextFieldPayDCity.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 50) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDCity.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayDProv.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 20) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayDProv.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jComboBoxPayAccountD.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				if (((JComboBox) evt.getSource()) != null && ((JComboBox) evt.getSource()).getSelectedItem() != null
					...
					String query = "SELECT IBAN,BANK_CODE,BRANCH_CODE,ADDRESS FROM ACCOUNT WHERE ACCOUNT.ACCOUNT_ID='"
						+ valueId + "'";
					SQLQuery s = FacturaeManager.getInstance().executeQuery(query);
					List<?> ls = s.list();
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						...
						if (values[3] != null) {
							String query2 = "SELECT ADDRESS,TOWN,POST_CODE,PROVINCE,COUNTRY FROM ADDRESS WHERE ADDRESS_ID='"
							...
							if (ls2 != null && ls2.size() > 0 && ls2.get(0) != null) {
								...
								if (values2[4] != null)
									jComboBoxPayDCoun.setSelectedIndex(ComboUtil.calculateComboIndex(
										Constants.COUNTRIES31, String.valueOf(values2[4])));
							}
						}
		...
		jPanel26Layout.setHorizontalGroup(jPanel26Layout.createSequentialGroup()
				.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel26Layout.createSequentialGroup()
								.addContainerGap()
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jComboBoxPayAccountD, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 160,
									Short.MAX_VALUE).add(10, 10, 10))
						.add(jPanel26Layout.createSequentialGroup().addContainerGap()
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextAreaAccExist, 0, 260, Short.MAX_VALUE)))
				.add(jSeparator2, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 2,
					org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
				.add(10, 10, 10)
				.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel26Layout.createSequentialGroup().add(jLabelTotalPayDIBAN)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDIBAN, 190, 190, 190))
						.add(jPanel26Layout.createSequentialGroup()
								.add(jLabelTotalPayDBank)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDBank, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 50,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayDBran)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDBran, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 55,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)))
				.add(15, 15, 15)
				.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel26Layout.createSequentialGroup()
								...
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDAddr, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 130,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayDCity)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDCity, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 110,
									Short.MAX_VALUE))
						.add(jPanel26Layout.createSequentialGroup()
								.add(jLabelTotalPayDPCod)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDPCod, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 45,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayDProv)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayDProv, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 90,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayDCoun)
								.add(jComboBoxPayDCoun, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 100,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))).addContainerGap());

		jPanel26Layout.setVerticalGroup(jPanel26Layout
			.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanel26Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER).add(
							jComboBoxPayAccountD, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
							org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
							org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTextAreaAccExist, 0, 260, Short.MAX_VALUE))
			.add(org.jdesktop.layout.GroupLayout.TRAILING, jPanel26Layout.createSequentialGroup()
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
					.add(jSeparator2, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 40,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addContainerGap())
			.add(jPanel26Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jLabelTotalPayDIBAN)
							.add(jTextFieldPayDIBAN, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDAddr)
							.add(jTextFieldPayDAddr, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDCity)
							.add(jTextFieldPayDCity, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jPanel26Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jLabelTotalPayDBank)
							.add(jTextFieldPayDBank, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDBran)
							.add(jTextFieldPayDBran, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDPCod)
							.add(jTextFieldPayDPCod, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDProv)
							.add(jTextFieldPayDProv, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayDCoun)
							.add(jComboBoxPayDCoun, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
		...
		jTextFieldPayCIBAN.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() < 5 && value.length() > 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCIBAN.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooLow"), Constants.ERROR_MSG_COLOR);
				}
				if (value.length() > 34) {
					((JTextField) evt.getSource()).setText("");
		...
		jTextFieldPayCBank.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCBank.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayCBran.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 60) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCBran.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayCAddr.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 80) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCAddr.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayCCity.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 50) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCCity.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
		...
		jTextFieldPayCPCod.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() != 5 && value.length() != 0) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCPCod.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamPostCodeLength"), Constants.ERROR_MSG_COLOR);
					return;
				}
				try {
		...
		jTextFieldPayCProv.addFocusListener(new FocusAdapter() {
			public void focusLost(FocusEvent evt) {
				...
				if (value.length() > 20) {
					((JTextField) evt.getSource()).setText("");
					((GenerateInvoice321Window) jTextFieldPayCProv.getTopLevelAncestor()).showMessage(
						Constants.LANG.getString("NOOKParamTooHigh"), Constants.ERROR_MSG_COLOR);
				}
			}
		});
					...
					if (ls != null && ls.size() > 0 && ls.get(0) != null) {
						...
						if (values[3] != null) {
							...
							if (ls2 != null && ls2.size() > 0 && ls2.get(0) != null) {
								...
								if (values2[4] != null)
									jComboBoxPayCCoun.setSelectedIndex(ComboUtil.calculateComboIndex(
										Constants.COUNTRIES31, String.valueOf(values2[4])));
							}
						}
					}
		...
		jPanel27Layout
			.setHorizontalGroup(jPanel27Layout
				...
				.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel27Layout.createSequentialGroup()
								.addContainerGap()
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jComboBoxPayAccountC, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 160,
									Short.MAX_VALUE).add(10, 10, 10))
						.add(jPanel27Layout.createSequentialGroup().addContainerGap()
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextAreaAccExist2, 0, 260, Short.MAX_VALUE)))
				.add(jSeparator3, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 2,
					org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
				.add(10, 10, 10)
				.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel27Layout.createSequentialGroup().add(jLabelTotalPayCIBAN)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCIBAN, 190, 190, 190))
						.add(jPanel27Layout.createSequentialGroup()
								.add(jLabelTotalPayCBank)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCBank, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 50,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayCBran)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCBran, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 55,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)))
				.add(15, 15, 15)
				.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
						.add(jPanel27Layout.createSequentialGroup()
								...
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCAddr, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 130,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayCCity)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCCity, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 110,
									Short.MAX_VALUE))
						.add(jPanel27Layout.createSequentialGroup()
								.add(jLabelTotalPayCPCod)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCPCod, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 45,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayCProv)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jTextFieldPayCProv, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 90,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
								.add(jLabelTotalPayCCoun)
								.add(jComboBoxPayCCoun, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 100,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))).addContainerGap());
		...
		jPanel27Layout.setVerticalGroup(jPanel27Layout
			...
			.add(jPanel27Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER).add(
							jComboBoxPayAccountC, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
							org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
							org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTextAreaAccExist2, 0, 260, Short.MAX_VALUE))
			.add(org.jdesktop.layout.GroupLayout.TRAILING, jPanel27Layout.createSequentialGroup()
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
					.add(jSeparator3, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 40,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addContainerGap())
			.add(jPanel27Layout.createSequentialGroup()
					.add(5, 5, 5)
					.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jLabelTotalPayCIBAN)
							.add(jTextFieldPayCIBAN, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCAddr)
							.add(jTextFieldPayCAddr, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCCity)
							.add(jTextFieldPayCCity, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jPanel27Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jLabelTotalPayCBank)
							.add(jTextFieldPayCBank, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCBran)
							.add(jTextFieldPayCBran, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCPCod)
							.add(jTextFieldPayCPCod, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCProv)
							.add(jTextFieldPayCProv, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jLabelTotalPayCCoun)
							.add(jComboBoxPayCCoun, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)));
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if(hasSelectedAccount) {
	                ...
	                if(selRow > -1) {
						...
						SQLQuery sInvoice = FacturaeManager.getInstance().executeQuery("SELECT INVOICE_ID FROM INVOICE WHERE NVL(SERIES_CODE+NUMBER,NUMBER) = '"+ originalId + "'");
...
// Inerza S.A. - Añadir endosatario v321 - #0062026
		...
		jPanel28Layout.setHorizontalGroup(jPanel28Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(org.jdesktop.layout.GroupLayout.TRAILING,jPanel28Layout.createSequentialGroup()
					.addContainerGap(org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTabbedPaneGlbPayment, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 931,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).add(13, 13, 13)));
		jPanel28Layout.setVerticalGroup(jPanel28Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanel28Layout.createSequentialGroup().add(
						jPanel28Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
							.add(jPanel28Layout.createSequentialGroup().addContainerGap())
							.add(jPanel28Layout.createSequentialGroup()
									.add(4, 4, 4)
									.add(jTabbedPaneGlbPayment, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, 83,
		...
		jPanel29Layout.setHorizontalGroup(jPanel29Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanel29Layout.createSequentialGroup()
					.add(56, 56, 56)
					.add(jLabelTotalWithItem, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 85,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTextFieldWithItem, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 84,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(79, 79, 79)
					.add(jLabelTotalWithPerc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 96,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTextFieldWithPerc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 92,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.add(127, 127, 127)
					.add(jLabelTotalWithAmou, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 81,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTextFieldWithAmou, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 118,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).addContainerGap(124, Short.MAX_VALUE)));
		jPanel29Layout.setVerticalGroup(jPanel29Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanel29Layout.createSequentialGroup()
					.add(35, 35, 35)
					.add(jPanel29Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jTextFieldWithItem, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jTextFieldWithPerc, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jTextFieldWithAmou, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
								org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).add(jLabelTotalWithItem)
							.add(jLabelTotalWithPerc).add(jLabelTotalWithAmou)).addContainerGap(44, Short.MAX_VALUE)));
        ...
        jTextAreaLegal.setAlignmentY(Component.TOP_ALIGNMENT);				
		jTextAreaLegal.setLineWrap(true);
		jTextAreaLegal.setColumns(100);
        jTextAreaLegal.setRows(2);
		jTextAreaLegal.setLineWrap(true);
		...
		jButtonLitItemAdd.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				if(!literalsListDestModel.contains(jTextAreaLegal.getText()))
					literalsListDestModel.addElement(jTextAreaLegal.getText());
			}
		});
			...
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				if (jListLiteralsDest.getSelectedIndices().length > 0) {
					...
					for (int i = selectedIndices.length-1; i >=0; i--) {
						...
						literalsListDestModel.removeElementAt(currentIndexes[i]);
					}
				}
			}
		});	
		...
		jTableItems.setModel(new NoEdiTableModel(new Object[][] {}, 
			new String[] {
		...
		jTableItems.getColumnModel().getColumn(0).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableItems.getColumnModel().getColumn(1).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		jTableItems.getColumnModel().getColumn(2).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.LEFT));
		jTableItems.getColumnModel().getColumn(3).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		jTableItems.getColumnModel().getColumn(4).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		jTableItems.getColumnModel().getColumn(5).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		jTableItems.getColumnModel().getColumn(6).setCellRenderer(
				new CustomCellRenderer(Constants.SELECTION_COLOR, java.awt.Color.white, Constants.FONT_COLOR,
					Constants.FONT_COLOR, Constants.FONT_PLAIN, javax.swing.SwingConstants.RIGHT));
		...
		jLabelTOTTaxBase2.setText(Constants.LANG.getString("AmountBeforeTaxes"));
		...
		jLabelTOTTaxBase.setText(Constants.LANG.getString("AmountBeforeTaxes"));
				...
				if (((JTextField) evt.getSource()) != null && ((JTextField) evt.getSource()).getText() != null) {
					if (!((JTextField) evt.getSource()).getText().trim().equals("")) {
						try {
							...
							((JTextField) evt.getSource()).setText(DoubleUtil.roundWithPrecisionDoubleFormatted(d,
								Integer.parseInt(decimalProps.getProperty("item_financial_expenses"))));
						} catch (NumberFormatException nfe) {
							((JTextField) evt.getSource()).setText("");
							((JTextField) evt.getSource()).setForeground(Constants.BKG_ERROR_COLOR);
							((GenerateInvoice321Window) jTextFieldWithItem.getTopLevelAncestor()).showMessage(
		...
		jButtonTOTXMLAddData.addActionListener(new java.awt.event.ActionListener() {
			...
			private void verifyXML(ActionEvent evt) {
				if (jTextAreaTOTXMLExtAddData.getText() == null
					|| "".equals(jTextAreaTOTXMLExtAddData.getText().trim())) {
					error2 = "";
					jButtonTOTXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
						"/images/button_mini_zoom.jpg")));
				} else {
					// Valid Invoice 3.1
					InputStream is = this.getClass().getResourceAsStream(Constants.I321_EMPTY_PATH);
					...
					try {
						...
						((GenerateInvoice321Window) jButtonTOTXMLAddData.getTopLevelAncestor()).showMessage(
							Constants.LANG.getString("NOOKMessageExtensionAdded"), Constants.ERROR_MSG_COLOR);
					} catch (FileNotFoundException e) {
						jButtonTOTXMLAddDataZoom.setIcon(new ImageIcon(getClass().getResource(
							"/images/button_mini_zoom_error.jpg")));
		...
		jButtonTOTXMLAddDataZoom.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent evt) {
				InputExtensionDialog ied = new InputExtensionDialog((JFrame) jTextAreaTOTXMLExtAddData
					.getTopLevelAncestor(), true, jTextAreaTOTXMLExtAddData.getText(), error2);
				ied.setVisible(true);
		...
		jPanel4Layout.setHorizontalGroup(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
				.add(jPanel4Layout.createSequentialGroup()
						...
						.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
								.add(jTabbedPaneGlobal, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 970,
									org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
								.add(org.jdesktop.layout.GroupLayout.LEADING,jPanel4Layout.createSequentialGroup()
										.add(jScrollPaneItems, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 900,
											org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
										.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
										.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING, false)
												.add(jButtonItemAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
												.add(jButtonItemDel, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)).add(16, 16, 16))
								.add(org.jdesktop.layout.GroupLayout.TRAILING,jPanel4Layout.createSequentialGroup()
										...
										.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
												...
												.add(jPanel4Layout.createSequentialGroup()
														...
														.add(jLabelTOTTaxBase,
															...
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
														.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
														.add(jTextFieldResults[3],
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
															textFieldTOTsize,
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
														.add(jLabelCurrency[3])															
														.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
														.add(jLabelTOTAsterisk,
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
															org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
												.add(jPanel4Layout.createSequentialGroup()
										...
										.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
												.add(jPanel4Layout.createSequentialGroup().add(
														jPanel4Layout.createParallelGroup(
																org.jdesktop.layout.GroupLayout.LEADING)
															.add(jLabelTOTXMLAddData)
															.add(jTextAreaTOTXMLExtAddData,
																org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 130,
																org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))))
										.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
												.add(jButtonTOTXMLAddData,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
												.add(jButtonTOTXMLAddDataZoom,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
													org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))))
						.addContainerGap()));
		...
		jPanel4Layout.setVerticalGroup(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.LEADING)
			.add(jPanel4Layout.createSequentialGroup()
					.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.TRAILING)
							.add(jScrollPaneItems, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 67,
								org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
							.add(jPanel4Layout.createSequentialGroup()
									.add(jButtonItemAdd, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
										org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
									.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED).add(jButtonItemDel)))
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jTabbedPaneGlobal, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE, 126,
						org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
					.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
					.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
							.add(jPanel4Layout.createSequentialGroup()
									.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
											.add(jLabelTOTALS, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
											.add(jLabelTOTMessage, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE))
									.add(6, 6, 6)
									.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
											.add(jTextFieldResults[0], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTMinus1)
											.add(jTextFieldResults[1], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTPlus1)
											.add(jTextFieldResults[2], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTEqual1)
											.add(jTextFieldResults[3], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTAsterisk, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTGrossAmount).add(jLabelTOTDiscounts).add(jLabelTOTCharges)
											.add(jLabelTOTTaxBase)
											.add(jLabelCurrency[0]).add(jLabelCurrency[1])
											.add(jLabelCurrency[2]).add(jLabelCurrency[3])
											)
									.addPreferredGap(org.jdesktop.layout.LayoutStyle.RELATED)
									.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
											.add(jLabelTOTPlus2)
											.add(jTextFieldResults[4], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTMinus2)
											.add(jTextFieldResults[5], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTEqual2)
											.add(jLabelTOTTaxBase2)
											...
											.add(jLabelTOTTaxWith)
											.add(jLabelTOTInvTotal)
											.add(jTextFieldResults[6], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelCurrency[4])
											.add(jLabelCurrency[5])
									...
									.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
											.add(jLabelTOTMinus3)
											.add(jTextFieldResults[7], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTPlus3)
											.add(jTextFieldResults[8], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												19, org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTPlus4)
											.add(jLabelTOTInvTotal2)
											.add(jLabelTOTGlobWith)
											.add(jLabelTOTReimb)
											.add(jTextFieldResults[9], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE)
											.add(jLabelTOTEqual3)
											.add(jTextFieldResults[10], org.jdesktop.layout.GroupLayout.PREFERRED_SIZE,
												org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
												org.jdesktop.layout.GroupLayout.PREFERRED_SIZE).add(jLabelTOTInvExec)
											.add(jLabelTOTFinancial)
											.add(jLabelCurrency[7]).add(jLabelCurrency[8])
											.add(jLabelCurrency[9]).add(jLabelTCurrency)
											))
							.add(jPanel4Layout.createSequentialGroup()
									.add(jLabelTOTXMLAddData, org.jdesktop.layout.GroupLayout.DEFAULT_SIZE,
										org.jdesktop.layout.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
									.add(jPanel4Layout.createParallelGroup(org.jdesktop.layout.GroupLayout.CENTER)
											.add(jTextAreaTOTXMLExtAddData,
		...
		this.setBorder(BorderFactory.createTitledBorder(new LineBorder(Constants.SELECTION_COLOR, 1, true),
			Constants.LANG.getString("InvoiceDetailData"), TitledBorder.DEFAULT_JUSTIFICATION,
			TitledBorder.DEFAULT_POSITION, Constants.FONT_BOLD, Constants.FONT_COLOR));
	...
	private void loadDecimalConfiguration() {
		try {
			...
			logger.error("It is not possible to obtain the decimals 3.2.1 properties file: " + ioe.getMessage());
		}
	}
		...
		try {
			...
			if (ls != null && ls.size() > 0 && ls.get(0) != null) {
				...
				for (int i = 0; i < count; i++)
					payments.addElement(new String(Constants.LANG.getString("PaymentDetails") + ": "
						+ ((Integer) ls.get(i)).toString()));
				payments.addElement("");
			}
			...
			if (ls != null && ls.size() > 0 && ls.get(0) != null) {
				int count = ls.size();
				for (int i = 0; i < count; i++) {
					String elem = new String(((Object[]) ls.get(i))[0].toString() + ": "
						+ ((Object[]) ls.get(i))[1].toString());
	...
	private void addDiscount(java.awt.event.ActionEvent evt) {
		...
		if (tibString != null && !"".equals(tibString)) {
			try {
				...
				tib = DoubleUtil.roundWithPrecisionDouble(tib, 8);
				tot = DoubleUtil.getDoubleFromPrettyNumber(totString);
				tot = DoubleUtil.roundWithPrecisionDouble(tot, 8);
			} catch (NumberFormatException e) {
				logger.error("An number format exception occurred while a discount was being added: " + e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
					Constants.LANG.getString("NOOKParamNumericFormat"), Constants.ERROR_MSG_COLOR);
				return;
			}
		}
		if ((tib == null || "".equals(tib)) || (tot == null || "".equals(tot))) { // If gross amount or total cost does not have a positive value
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKParamItemRequired"), Constants.ERROR_MSG_COLOR);
			return;
		}
		InputDiscountDialog igd = new InputDiscountDialog((JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true, tot,
			tib, Constants.VERSION321, true); // Tot is the source, tib is the value that may not exceed
		igd.setVisible(true);
		String[] result = igd.getValues();
		if (result == null || result.length != 3) {
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageCanceledByUser"), Constants.OK_MSG_COLOR);
			return;
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageDiscountAdded"), Constants.OK_MSG_COLOR);
		...
		if (item != null && !"".equals(item)) {
			...
			jTextFieldTotalDisc.setText(DoubleUtil.roundWithPrecisionDoubleFormatted((totDiscount + Double.valueOf(amount)),
				Integer.parseInt(decimalProps.getProperty("item_discount"))));
	...
	private void delDiscount(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			try {
				if (jTextFieldTotalDisc != null && jTextFieldTotalDisc.getText() != null
					&& !jTextFieldTotalDisc.getText().trim().equals(""))
					totalDiscount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldTotalDisc.getText());
				jTextFieldTotalDisc.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
					(totalDiscount - DoubleUtil.getDoubleFromPrettyNumber(((NoEdiTableModel) jTableGlobDisc.getModel())
						.getValueAt(row, 2).toString())), Integer.parseInt(decimalProps.getProperty("item_discount"))));
			} catch (NumberFormatException e) {
				logger.error("An number format exception occurred while a discount was being deleted: "
					+ e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
					Constants.LANG.getString("NOOKParamNumericFormat"), Constants.ERROR_MSG_COLOR);
				return;
			...
			recalculateTotals();
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageDiscountDeleted"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKForDeleteMessage"), Constants.ERROR_MSG_COLOR);
	}
	...
	private void addReim(java.awt.event.ActionEvent evt) {
		...
		if (tibString == null || "".equals(tibString.trim())) {
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				"Debe facturar algun concepto antes de aplicar suplidos", Constants.ERROR_MSG_COLOR);
			return;
		}
		...
		if (result == null || result.length != 7) {
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKIncompleteForm"), Constants.ERROR_MSG_COLOR);
			return;
		}
		String amount = result[6];
		if (amount != null && !"".equals(amount)) {// If not, it was canceled by user
			((NoEdiTableModel) jTableGlobReim.getModel()).addRow(new Object[] { result[0], result[1], result[2],
			...
			jTextFieldTotalReim.setText(DoubleUtil.roundWithPrecisionDoubleFormatted((totValue + doubleAmount),
				Integer.parseInt(decimalProps.getProperty("item_total_reim_amount"))));
			jTextFieldResults[8].setText(DoubleUtil.roundWithPrecisionDoubleFormatted((totValue + doubleAmount),
				Integer.parseInt(decimalProps.getProperty("item_total_reim_amount"))));
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageReimAdded"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageCanceledByUser"), Constants.OK_MSG_COLOR);
		recalculateTotals();
	...
	private void delReim(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			try {
				...
				logger.error("An number format exception occurred while a reimbursable expense was being deleted: "
					+ e.getMessage());
				((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
					Constants.LANG.getString("NOOKParamNumericFormat"), Constants.ERROR_MSG_COLOR);
				return;
			}
			jTextFieldTotalReim.setText(DoubleUtil.roundWithPrecisionDoubleFormatted((totValue - lineAmount),
				Integer.parseInt(decimalProps.getProperty("item_total_reim_amount"))));
			jTextFieldResults[8].setText(DoubleUtil.roundWithPrecisionDoubleFormatted((totValue - lineAmount),
				Integer.parseInt(decimalProps.getProperty("item_total_reim_amount"))));
			((NoEdiTableModel) jTableGlobReim.getModel()).removeRow(row);
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageReimDeleted"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKForDeleteMessage"), Constants.ERROR_MSG_COLOR);
		recalculateTotals();
	...
	private void addAttachment(java.awt.event.ActionEvent evt) {
		...
		InputAttachmentDialog iad = InputAttachmentDialog.getInstance(
			(JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), prevDesc, true, includedLength);
		iad.setVisible(true);
		...
		if (format != null && !"".equals(format) && document != null && !"".equals(document)) {// If not, it was canceled
			((NoEdiTableModel) jTableGlobAttc.getModel()).addRow(new Object[] { format, document, description,
					includingXML });
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageAttachAdded"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageCanceledByUser"), Constants.OK_MSG_COLOR);
	}
	...
	private void delAttachment(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMesaggeAttachDeleted"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKForDeleteMessage"), Constants.ERROR_MSG_COLOR);
	}
	...
	private void addItem(java.awt.event.ActionEvent evt, InvoiceLineType lineToEdit, int lineNumber) {
		Invoice321DetailDialog idd = null;
		if (jTableItems.getModel().getRowCount() == 0)
			idd = new Invoice321DetailDialog((JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true, null, false);
		else
			idd = new Invoice321DetailDialog((JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true,
				((GenerateInvoice321Window) this.getTopLevelAncestor()).specialTB, setVisualizationOnly);
		if (lineToEdit != null)
			idd.setValues((es.mityc.appfacturae.facturae321.InvoiceLineType) lineToEdit);
		idd.setVisible(true);
		InvoiceLineType ilt = idd.getValues();
		
		//ob es una fila del grid con cada linea de pedido.
		//Los campos de ob son DESCRIPTION QUANTITY UNIT_OF_MEASURE,TOTAL_COST,POS_TAX_OUT,POS_TAX_WITH,ITEM_TOTAL_COST
		Object[] ob = new Object[] { "", "", "", "", "", "", "" };
		if (ilt.getItemDescription() != null && !ilt.getItemDescription().trim().equals("") && ilt.getQuantity() != 0.0
			&& !"".equals(ilt.getQuantity())) { // If not, it was canceled by user
			ob[Constants.POS_ITEM_DESCRIPTION_GRID] = ilt.getItemDescription();
			ob[Constants.POS_QUANTITY_GRID] = DoubleUtil.formatDecimal(ilt.getQuantity(),-1);
			if (ilt.getUnitOfMeasure() != null && !ilt.getUnitOfMeasure().trim().equals("")){
				...
				ob[Constants.POS_UNIT_OF_MEASURE_GRID] = Constants.LANG.getString(unitsOfMeasure[Integer.parseInt(ilt.getUnitOfMeasure()) - 1]);
			}
			ob[Constants.POS_TOTAL_COST_GRID] = DoubleUtil.formatDecimal(ilt.getTotalCost(),Integer.parseInt(decimalProps.getProperty("item_total_cost")));
			int numTaxes = 0;
			if (((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt).getTaxesOutputs() != null
				&& ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt).getTaxesOutputs().getTax() != null)
				numTaxes = ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt).getTaxesOutputs().getTax().size();
			String typeTax = "";
			boolean different = false;
			double amountTax = 0.00;
			for (int i = 0; i < numTaxes; i++) {
				Tax t = ((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt).getTaxesOutputs().getTax().get(i);
				if (!different) {
					if (typeTax.equals(""))
				...
				if (t.getSpecialTaxAmount() != null && t.getSpecialTaxAmount().getTotalAmount() > 0.00) {
					...
					((GenerateInvoice321Window) this.getTopLevelAncestor()).specialTB = true;
				} else
					amountTax = amountTax + t.getTaxAmount().getTotalAmount();

				if (t.getEquivalenceSurchargeAmount() != null
					&& t.getEquivalenceSurchargeAmount().getTotalAmount() != 0)
			...
			if (numTaxes > 0) {
				if (!"".equals(typeTax) && !Constants.LANG.getString("DifferentValues").equals(typeTax)) {
					typeTax = Constants.LANG.getString(Constants.APP_PROP.getProperty("taxesType" + codeVersion).split(";")
							[Integer.valueOf(typeTax) - 1]).split(" ")[0];
				}
				ob[Constants.POS_TAX_OUT_GRID] = typeTax + " - " + DoubleUtil.roundWithPrecisionDoubleFormatted(amountTax,
						Integer.parseInt(decimalProps.getProperty("item_tax_Out")));
			} else
				ob[Constants.POS_TAX_OUT_GRID] = Constants.LANG.getString(Constants.APP_PROP.getProperty("specialTaxableEvent").split(";")
						[Integer.parseInt(((es.mityc.appfacturae.facturae321.InvoiceLineType) ilt).getSpecialTaxableEvent()
			...
			if (numTaxesW > 0) {
				if (!"".equals(typeTaxW) && !Constants.LANG.getString("DifferentValues").equals(typeTaxW)) {
					typeTaxW = Constants.LANG
						.getString(
							Constants.APP_PROP.getProperty("taxesType" + codeVersion).split(";")[Integer
								.valueOf(typeTaxW) - 1]).split(" ")[0];
				}
				ob[Constants.POS_TAX_WITH_GRID] = typeTaxW + " - " + DoubleUtil.roundWithPrecisionDoubleFormatted(amountTaxW,
						Integer.parseInt(decimalProps.getProperty("item_tax_With")));
			} else
				ob[Constants.POS_TAX_WITH_GRID] = "";

			
			Double itemGrossCost = DoubleUtil.roundWithPrecisionDouble(ilt.getGrossAmount(), Integer.parseInt(decimalProps.getProperty("item_gross_cost")));
			Double doubleObj = (Double.valueOf(itemGrossCost.toString()) + amountTax) - amountTaxW;
			doubleObj = DoubleUtil.roundWithPrecisionDouble(doubleObj, Integer.parseInt(decimalProps.getProperty("item_total_cost")));
			ob[Constants.POS_ITEM_TOTAL_COST_GRID] = DoubleUtil.formatDecimal(doubleObj,Integer.parseInt(decimalProps.getProperty("item_total_cost")));
			...
			if (jTextFieldResults[0].getText() != null && !jTextFieldResults[0].getText().trim().equals(""))
				jTextFieldResults[0].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
					(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[0].getText()) + ilt.getGrossAmount()),
					Integer.parseInt(decimalProps.getProperty("total_gross_amount"))));
			else
				jTextFieldResults[0].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(ilt.getGrossAmount(),
					Integer.parseInt(decimalProps.getProperty("total_gross_amount"))));

			if (jTextFieldResults[4].getText() != null && !jTextFieldResults[4].getText().trim().equals(""))
				jTextFieldResults[4].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
					(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[4].getText()) + amountTax),
					Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));
			else
				jTextFieldResults[4].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(amountTax,
					Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));

			if (jTextFieldResults[5].getText() != null && !jTextFieldResults[5].getText().trim().equals(""))
				jTextFieldResults[5].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
					(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[5].getText()) + amountTaxW),
					Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
			else
				jTextFieldResults[5].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(amountTaxW,
					Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
			tibChanged();
			recalculateTotals();
		} else
		...
		if (row >= 0) {
			...
			for (int i = 0; i < numTaxes; i++) {
				...
				if (t.getSpecialTaxAmount() != null && t.getSpecialTaxAmount().getTotalAmount() > 0.00) {
					...
					if (jTableItems.getRowCount() == 1) {
						((GenerateInvoice321Window) this.getTopLevelAncestor()).specialTB = false;
			...
			if (amountTaxObject != null && !"".equals(amountTaxObject.toString())) {
				...
				if (amountTaxArray[amountTaxArray.length - 1] != null
					...
					double amountTax = DoubleUtil.getDoubleFromPrettyNumber(amountTaxArray[amountTaxArray.length - 1]);
					jTextFieldResults[4].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
						(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[4].getText()) - amountTax),
						Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));
				}
			...
			if (amountTaxWObject != null && !"".equals(amountTaxWObject.toString())) {
				...
				if (amountTaxWArray[amountTaxWArray.length - 1] != null
					...
					double amountTaxW = DoubleUtil.getDoubleFromPrettyNumber(amountTaxWArray[amountTaxWArray.length - 1]);
					jTextFieldResults[5].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
						(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[5].getText()) - amountTaxW),
						Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
				}
	...
	private void addCharge(java.awt.event.ActionEvent evt) {
		...
		if (tibString != null && !"".equals(tibString)) {
			try {
				tib = DoubleUtil.getDoubleFromPrettyNumber(tibString);
				tib = DoubleUtil.roundWithPrecisionDouble(tib, 2);
			} catch (NumberFormatException e) {
		...
		if (tib == null || "".equals(tib)) {
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				"Debe facturar algun concepto antes de aplicar cargos", Constants.ERROR_MSG_COLOR);
			return;
		}
		InputChargeDialog igd = new InputChargeDialog((JFrame) jPanelGlbDiscounts.getTopLevelAncestor(), true, tib,
			Constants.VERSION321, true);
		igd.setVisible(true);
		String[] result = igd.getValues();
		...
		if (item != null && !"".equals(item)) {// En caso contrario, se cancelo
			((NoEdiTableModel) jTableGlobChar.getModel()).addRow(new Object[] { item, rate, amount });
			String strTotCharge = jTextFieldTotalChar.getText();
			if (strTotCharge == null || "".equals(strTotCharge.trim()))
				strTotCharge = "0.0";
			Double totValue = Double.valueOf(strTotCharge);
			Double doubleAmount = Double.valueOf(amount);
			jTextFieldTotalChar.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totValue + doubleAmount,
				Integer.parseInt(decimalProps.getProperty("item_charge"))));
			jTextFieldResults[2].setText(jTextFieldTotalChar.getText());
			recalculateTotals();
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageChargeAdded"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageCanceledByUser"), Constants.OK_MSG_COLOR);
	}
	...
	private void delCharge(java.awt.event.ActionEvent evt) {
		...
		if (row >= 0) {
			...
			if (totValue < 0)
				totValue = 0.0;
			jTextFieldTotalChar.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totValue,
				Integer.parseInt(decimalProps.getProperty("item_charge"))));
			((NoEdiTableModel) jTableGlobChar.getModel()).removeRow(row);
			jTextFieldResults[2].setText(jTextFieldTotalChar.getText());
			recalculateTotals();
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("OKMessageChargeDeleted"), Constants.OK_MSG_COLOR);
		} else
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKForDeleteMessage"), Constants.ERROR_MSG_COLOR);
	}
	...
	public void tibChanged() {
		Double tib = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[0].getText());
		String value = null;
		Double d = 0.0;
		...
		for (int i = 0; i < modelDisc.getRowCount(); ++i) {
			...
			if (value != null && !"".equals(value.trim())) {
				...
				d = (d * tib) / 100;
				d = DoubleUtil.roundWithPrecisionDouble(d, Integer.parseInt(decimalProps.getProperty("item_discount")));
				totDis = totDis + d;
				modelDisc.setValueAt(d, i, 2);
			} else {
				d = DoubleUtil.getDoubleFromPrettyNumber(modelDisc.getValueAt(i, 2).toString());
				totDis = totDis + d;
			}
		}
		jTextFieldTotalDisc.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totDis,
			Integer.parseInt(decimalProps.getProperty("item_discount"))));
		jTextFieldResults[1].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totDis,
			Integer.parseInt(decimalProps.getProperty("item_discount"))));
		...
		for (int i = 0; i < modelChar.getRowCount(); ++i) {
			...
			if (value != null && !"".equals(value.trim())) {
				d = DoubleUtil.getDoubleFromPrettyNumber(value);
				d = (d * tib) / 100;
				d = DoubleUtil.roundWithPrecisionDouble(d, Integer.parseInt(decimalProps.getProperty("item_charge")));
				totChar = totChar + d;
				modelChar.setValueAt(d, i, 2);
			} else {
				d = DoubleUtil.getDoubleFromPrettyNumber(modelChar.getValueAt(i, 2).toString());
				totChar = totChar + d;
			}
		}
		jTextFieldTotalChar.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totChar,
			Integer.parseInt(decimalProps.getProperty("item_charge"))));
		jTextFieldResults[2].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(totChar,
			Integer.parseInt(decimalProps.getProperty("item_charge"))));
		...
		if (value != null && !"".equals(value.trim())) {
			d = DoubleUtil.getDoubleFromPrettyNumber(value);
			d = (d * tib) / 100;
			jTextFieldWithAmou.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(d,
				Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
			jTextFieldResults[7].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(d,
				Integer.parseInt(decimalProps.getProperty("item_tax_With"))));
		}
	}
	...
	public void recalculateTotals() {
		...
		if (jTextFieldResults[0].getText() != null && !jTextFieldResults[0].getText().trim().equals(""))
			grossAmount = DoubleUtil.roundWithPrecisionDouble(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[0].getText()),
				Integer.parseInt(decimalProps.getProperty("total_gross_amount")));
		double discountAmount = 0.0;
		if (jTextFieldTotalDisc.getText() != null && !jTextFieldTotalDisc.getText().trim().equals(""))
			discountAmount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldTotalDisc.getText());
		jTextFieldResults[1].setText(DoubleUtil.formatDecimal(discountAmount,Integer.parseInt(decimalProps.getProperty("discounts_total"))));
		double chargeAmount = 0.0;
		if (jTextFieldTotalChar.getText() != null && !jTextFieldTotalChar.getText().trim().equals(""))
			chargeAmount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldTotalChar.getText());
		jTextFieldResults[2].setText(DoubleUtil.formatDecimal(chargeAmount,Integer.parseInt(decimalProps.getProperty("charges_total"))));
		// BI
		Double biAmount = DoubleUtil.roundWithPrecisionDouble(grossAmount - discountAmount + chargeAmount,
			Integer.parseInt(decimalProps.getProperty("item_taxable_base")));
		jTextFieldResults[3].setText(DoubleUtil.formatDecimal(biAmount,Integer.parseInt(decimalProps.getProperty("item_taxable_base"))));

		//20150330: Calculamos porcentaje de descuento
		// Se establece el porcentaje a descontar calculado a partir del total
		Double discountsTotalRate = (100 * discountAmount) / grossAmount ;
		
		//Calculamos porcentaje de cargo
		Double chargesTotalRate = (100 * chargeAmount) / grossAmount ;
		...
		for (int i = 0; i < itemsSize; i++) {
			// Importe bruto de la línea actual
			gross2Amount = items.getInvoiceLine().get(i).getGrossAmount();
			//bi2Amount = gross2Amount - discountsTotal + chargesTotal;
			...
			int taxesOutputsSize = ((es.mityc.appfacturae.facturae321.InvoiceLineType) items.getInvoiceLine().get(i)).getTaxesOutputs().getTax().size();
			for (int j = 0; j < taxesOutputsSize; j++) {
				es.mityc.appfacturae.facturae321.InvoiceLineType.TaxesOutputs.Tax tax = 
						((es.mityc.appfacturae.facturae321.InvoiceLineType) items.getInvoiceLine().get(i)).getTaxesOutputs().getTax().get(j);
			...
			if (items.getInvoiceLine().get(i).getTaxesWithheld() != null
				&& items.getInvoiceLine().get(i).getTaxesWithheld().getTax() != null)
				taxesWithheldSize = items.getInvoiceLine().get(i).getTaxesWithheld().getTax().size();
			for (int j = 0; j < taxesWithheldSize; j++) {
				es.mityc.appfacturae.facturae.TaxType tax = items.getInvoiceLine().get(i).getTaxesWithheld().getTax()
					.get(j);
		...
		for (int i = 0; i < taxAmountList.size(); i++) {
			impuestoActual = DoubleUtil.roundWithPrecisionDouble(taxAmountList.get(i), Integer
					.parseInt(decimalProps.getProperty("item_taxable_base")));
			
			outputsAmount += DoubleUtil.roundWithPrecisionDouble(impuestoActual, Integer
					.parseInt(decimalProps.getProperty("item_taxable_base")));
		}
	
		Double value = new Double(0.0);
		Iterator it = TaxesWithheldMap.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry e = (Map.Entry) it.next();
			value = DoubleUtil.getDoubleFromPrettyNumber(e.getValue().toString());
			value = DoubleUtil.roundWithPrecisionDouble(value, Integer.parseInt(decimalProps.getProperty("item_taxable_base")));
			withheldAmount += DoubleUtil.getDoubleFromPrettyNumber(e.getKey().toString().split(" - ")[1].toString()) / 100 * value;
		}

		outputsAmount = DoubleUtil.roundWithPrecisionDouble(outputsAmount, Integer.parseInt(decimalProps.getProperty("item_tax_Out")));
		withheldAmount = DoubleUtil.roundWithPrecisionDouble(withheldAmount, Integer.parseInt(decimalProps.getProperty("item_tax_With")));

		jTextFieldResults[4].setText(DoubleUtil.formatDecimal(outputsAmount,Integer.parseInt(decimalProps.getProperty("item_tax_Out"))));
		jTextFieldResults[5].setText(DoubleUtil.formatDecimal(withheldAmount, Integer.parseInt(decimalProps.getProperty("item_tax_With"))));

		double totalAmount = biAmount + outputsAmount - withheldAmount;
		try {
			totalAmount= DoubleUtil.roundWithPrecisionDouble(totalAmount,Integer.parseInt(decimalProps.getProperty("item_total_cost")));
			jTextFieldResults[6].setText(DoubleUtil.formatDecimal(totalAmount,Integer.parseInt(decimalProps.getProperty("item_total_cost"))));
		} catch (NumberFormatException nfe) {
		...
		if (jTextFieldWithPerc.getText() != null && !"".equals(jTextFieldWithPerc.getText().trim())) {
			jTextFieldWithAmou
				.setText(DoubleUtil.roundWithPrecisionDoubleFormatted(
					(DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[6].getText())
						* DoubleUtil.getDoubleFromPrettyNumber(jTextFieldWithPerc.getText()) / 100.0),
					Integer.parseInt(decimalProps.getProperty("global_withheld"))));
		}
		...
		if (jTextFieldResults[7].getText() != null && !jTextFieldResults[7].getText().trim().equals(""))
			glbwithheldAmount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[7].getText());
		double reimbursableAmount = 0.0;
		if (jTextFieldResults[8].getText() != null && !jTextFieldResults[8].getText().trim().equals(""))
			reimbursableAmount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[8].getText());
		double financialAmount = 0.0;
		if (jTextFieldResults[9].getText() != null && !jTextFieldResults[9].getText().trim().equals("")) {
			try {
				financialAmount = DoubleUtil.getDoubleFromPrettyNumber(jTextFieldResults[9].getText());
			} catch (NumberFormatException nfe) {
				financialAmount = 0.0;
		...
		try {
			jTextFieldResults[10].setText(DoubleUtil.roundWithPrecisionDoubleFormatted(executableAmount,
				Integer.parseInt(decimalProps.getProperty("item_total_cost"))));
		} catch (NumberFormatException nfe) {
			jTextFieldResults[10].setText("");
			((GenerateInvoice321Window) this.getTopLevelAncestor()).showMessage(
				Constants.LANG.getString("NOOKParamNumericFormat"), Constants.ERROR_MSG_COLOR);
		}
		// alignment - caret position
		for (int i = 0; i < jTextFieldResults.length; i++) {
			jTextFieldResults[i].setCaretPosition(0);
		}
	}

	public boolean validateInstallment() {

		boolean requiredGeneralFilled = (jTextFieldPaymentAmo.getText() != null && !"".equals(jTextFieldPaymentAmo
			.getText().trim()))
			|| (jComboBoxPayMean.getSelectedItem() != null && !"".equals(jComboBoxPayMean.getSelectedItem().toString()
				.trim()));
		boolean generalFilled = requiredGeneralFilled
			|| (jTextFieldPaymentRefIssuer.getText() != null && !"".equals(jTextFieldPaymentRefIssuer.getText().trim()))
			|| (jTextFieldPaymentRefReceiver.getText() != null && !"".equals(jTextFieldPaymentRefReceiver.getText()
				.trim()));

		boolean requiredDebitedFilled = (jTextFieldPayDIBAN.getText() != null && !"".equals(jTextFieldPayDIBAN
			.getText().trim()));
		boolean debitedAddressFilled = (jTextFieldPayDAddr.getText() != null && !"".equals(jTextFieldPayDAddr.getText()
			.trim()))
			|| (jTextFieldPayDCity.getText() != null && !"".equals(jTextFieldPayDCity.getText().trim()))
			|| (jTextFieldPayDPCod.getText() != null && !"".equals(jTextFieldPayDPCod.getText().trim()))
			|| (jTextFieldPayDProv.getText() != null && !"".equals(jTextFieldPayDProv.getText().trim()))
			|| (jComboBoxPayDCoun.getSelectedItem() != null && !"".equals(jComboBoxPayDCoun.getSelectedItem()
				.toString().trim()));
		boolean debitedFilled = requiredDebitedFilled
			|| (jTextFieldPayDBank.getText() != null && !"".equals(jTextFieldPayDBank.getText().trim()))
			|| (jTextFieldPayDBran.getText() != null && !"".equals(jTextFieldPayDBran.getText().trim()))
			|| debitedAddressFilled;

		boolean requiredCreditedFilled = (jTextFieldPayCIBAN.getText() != null && !"".equals(jTextFieldPayCIBAN
			.getText().trim()));
		boolean creditedAddressFilled = (jTextFieldPayCAddr.getText() != null && !"".equals(jTextFieldPayCAddr
			.getText().trim()))
			|| (jTextFieldPayCCity.getText() != null && !"".equals(jTextFieldPayCCity.getText().trim()))
			|| (jTextFieldPayCPCod.getText() != null && !"".equals(jTextFieldPayCPCod.getText().trim()))
			|| (jTextFieldPayCProv.getText() != null && !"".equals(jTextFieldPayCProv.getText().trim()))
			|| (jComboBoxPayCCoun.getSelectedItem() != null && !"".equals(jComboBoxPayCCoun.getSelectedItem()
				.toString().trim()));
		boolean creditedFilled = requiredCreditedFilled
			|| (jTextFieldPayCBank.getText() != null && !"".equals(jTextFieldPayCBank.getText().trim()))
			|| (jTextFieldPayCBran.getText() != null && !"".equals(jTextFieldPayCBran.getText().trim()))
			|| creditedAddressFilled;

		if (!generalFilled && !debitedFilled && !creditedFilled) {
			...
			if ((jTextFieldPaymentAmo.getText() == null || "".equals(jTextFieldPaymentAmo.getText().trim()))
				|| (jComboBoxPayMean.getSelectedItem() == null || "".equals(jComboBoxPayMean.getSelectedItem()
					.toString().trim()))) {
				// NO OK These are the Installment's required attributes
				if (jComboBoxPayMean.getSelectedItem() == null
					|| "".equals(jComboBoxPayMean.getSelectedItem().toString().trim())) {
				...
				if (debitedFilled) {
					if (!requiredDebitedFilled) {
						...
						if (debitedAddressFilled) {
							if (!((jTextFieldPayDAddr.getText() != null && !"".equals(jTextFieldPayDAddr.getText()
								.trim()))
								&& (jTextFieldPayDCity.getText() != null && !"".equals(jTextFieldPayDCity.getText()
									.trim()))
								&& (((jTextFieldPayDPCod.getText() != null && !"".equals(jTextFieldPayDPCod.getText()
									.trim()))) || (jComboBoxPayDCoun.getSelectedItem() != null && Constants.SPAINCODE31 != Integer
									.parseInt(((ComboOption) jComboBoxPayDCoun.getSelectedItem()).getValue())))
								&& (jTextFieldPayDProv.getText() != null && !"".equals(jTextFieldPayDProv.getText()
									.trim())) && (jComboBoxPayDCoun.getSelectedItem() != null && !""
								.equals(jComboBoxPayDCoun.getSelectedItem().toString().trim())))) {
								// NO OK The account to be debited address is filled but not all its attributes.
								if (jTextFieldPayDAddr.getText() == null
									|| "".equals(jTextFieldPayDAddr.getText().trim())) {
									jTextFieldPayDAddr.setBackground(Constants.BKG_ERROR_COLOR);
									jTextFieldPayDAddr.requestFocus();
								}
								if (jTextFieldPayDCity.getText() == null
									|| "".equals(jTextFieldPayDCity.getText().trim())) {
									jTextFieldPayDCity.setBackground(Constants.BKG_ERROR_COLOR);
									jTextFieldPayDCity.requestFocus();
								}
								if (jComboBoxPayDCoun.getSelectedItem() != null
									&& Constants.SPAINCODE31 == Integer.parseInt(((ComboOption) jComboBoxPayDCoun
								...
								if (jTextFieldPayDProv.getText() == null
									|| "".equals(jTextFieldPayDProv.getText().trim())) {
					...
					if (!requiredCreditedFilled) {
						...
						if (creditedAddressFilled) {
							if (!((jTextFieldPayCAddr.getText() != null && !"".equals(jTextFieldPayCAddr.getText().trim()))
								&& (jTextFieldPayCCity.getText() != null && !"".equals(jTextFieldPayCCity.getText().trim()))
								&& (((jTextFieldPayCPCod.getText() != null && !"".equals(jTextFieldPayCPCod.getText().trim()))) 
									|| (jComboBoxPayCCoun.getSelectedItem() != null && Constants.SPAINCODE31 != Integer
									.parseInt(((ComboOption) jComboBoxPayCCoun.getSelectedItem()).getValue())))
								&& (jTextFieldPayCProv.getText() != null && !"".equals(jTextFieldPayCProv.getText().trim())) 
									&& (jComboBoxPayCCoun.getSelectedItem() != null 
									&& !"".equals(jComboBoxPayCCoun.getSelectedItem().toString().trim())))) {
								...
								if (jComboBoxPayCCoun.getSelectedItem() != null
									&& Constants.SPAINCODE31 == Integer.valueOf(((ComboOption) jComboBoxPayCCoun
										.getSelectedItem()).getValue())
									&& (jTextFieldPayCPCod.getText() == null || "".equals(jTextFieldPayCPCod.getText()
										.trim()))) {
	...
	public void setJComboBoxPayAccountFactoring(JComboBox jComboBoxPayAccountFactoring) {
		this.jComboBoxPayAccountFactoring = jComboBoxPayAccountFactoring;
	}
	...
	public void setJTextFieldPayFactNifSociedad(JTextField jTextFieldPayFactNifSociedad) {
    ...
    private JPanel jPanelGlbPaymentAccFactoring = null;
    private DefaultComboBoxModel<String> accounts3 = null;
}
